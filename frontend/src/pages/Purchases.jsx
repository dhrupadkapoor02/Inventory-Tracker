  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-slate-900">Purchases</h1>
          <p className="mt-1 text-sm text-slate-500">Stock received from suppliers</p>
        </div>
        <button
          onClick={openCreate}
            ...
            {loading ? (
              ...
              purchases.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="px-5 py-3 font-medium text-slate-900">{p.supplier.name}</td>
                  <td className="px-5 py-3 text-slate-500">{p.items.length} item(s)</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="New purchase" maxWidth="max-w-2xl">
